...
When(/^I approve the smart contract$/, async function (): Promise<void> {
    await ApproveHelper.approveSmartContract(this.org1Network, Helper.oldOrg1Peer, this.label, '0.0.1', this.packageId);
    await ApproveHelper.approveSmartContract(this.org2Network, Helper.oldOrg2Peer, this.label, '0.0.1', this.packageId);
});
